...
class Bala(pygame.sprite.Sprite):
    def __init__(self, x, y, direction, scale=1.0):  # Adicione o parâmetro scale
        super().__init__()
        self.image = bullet_img
       
        # Redimensiona a imagem da bala com base no parâmetro scale
...
class Soldado(pygame.sprite.Sprite):
    ...
    def shoot(self):
        if self.shoot_cooldown == 0 and self.ammo > 0:
            ...
            bullet = Bala(self.rect.centerx + (0.6 * self.rect.size[0] * self.direction),
                          self.rect.centery,
                          self.direction,
                          scale = 0.1)
